...
class FileWidget(QtWidgets.QWidget):
    ...
    def _project_actions():
        ...
        toolbar = util.right_justified_toolbar()
        toolbar.hide()
        toolbar.setIconSize(QtCore.QSize(20, 20))
        toolbar.addAction(util.icon("mdi.share", color="#efefef"), "Share Project")
        toolbar.addAction(util.icon("mdi.pencil", color="#efefef"), "Edit Project")
        toolbar.addAction(
            util.icon("mdi.dots-horizontal", color="#efefef"), "More Options"
        )
...
class FileTable(QtWidgets.QTableWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.rowCount() == 0:  # If the table is empty:
            ...
            icon = QtGui.QIcon(opts.CLIMACT_CONFIG["root"] + "/assets/empty.png")
